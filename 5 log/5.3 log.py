...
while True:
    ...
    # Находим изолированные, концевые и доминирующие вершины на основе матрицы инцидентности
    isolated_vertices_incidence = [i for i in range(graph_size) if sum(incidence_matrix[i][j] == 1 for j in range(num_edges_from_incidence)) == 0]
    end_vertices_incidence = [i for i in range(graph_size) if sum(incidence_matrix[i][j] == 1 for j in range(num_edges_from_incidence)) == 1]
    dominating_vertices_incidence = [i for i in range(graph_size) if sum(incidence_matrix[i][j] == 1 for j in range(num_edges_from_incidence)) == edge_count]

    # Выводим информацию о графе, включая вершины из матрицы инцидентности
    ...
    print("Изолированные вершины (из матрицы инцидентности) :", isolated_vertices_incidence)
    print("Концевые вершины (из матрицы инцидентности) :", end_vertices_incidence)
    print("Доминирующие вершины (из мататрицы инцидентности) :", dominating_vertices_incidence)
